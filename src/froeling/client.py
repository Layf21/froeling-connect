...
from .datamodels import Component, Facility, NotificationDetails, NotificationOverview, UserData
from .endpoints import FACILITY, NOTIFICATION, NOTIFICATION_COUNT, NOTIFICATION_LIST, USER
...
class Froeling:
    ...
    def __init__(
        ...
        Either username and password or a token is required.
                ...
                :param auto_reauth: Automatically fetches a new token if the current one expires
                    (requires password and username).
                :param max_retries: How often to retry a request if the request failed.
                :param token_callback: A function that is called when the token gets renewed
                    (useful for saving the token).
    ...
    async def login(self) -> UserData:
        data = await self.session.login()
        self._userdata = UserData.from_dict(data)
    ...
    async def _get_userdata(self) -> UserData:
        res = await self.session.request("get", USER.format(self.session.user_id))
        return UserData.from_dict(res)
    ...
    async def _get_facilities(self) -> list[Facility]:
        ...
        res = await self.session.request("get", FACILITY.format(self.session.user_id))
        return Facility.from_list(res, self.session)
    ...
    async def get_facility(self, facility_id) -> Facility:
    ...
    async def get_notification_count(self) -> int:
        ...
        return (await self.session.request("get", NOTIFICATION_COUNT.format(self.session.user_id)))[
            "unreadNotifications"
        ]

    async def get_notifications(self) -> list[NotificationOverview]:
        res = await self.session.request("get", NOTIFICATION_LIST.format(self.session.user_id))
        return [NotificationOverview(n, self.session) for n in res]

    async def get_notification(self, notification_id: int) -> NotificationDetails:
        res = await self.session.request("get", NOTIFICATION.format(self.session.user_id, notification_id))
        return NotificationDetails.from_dict(res)

    def get_component(self, facility_id: int, component_id: str):
        return Component(facility_id, component_id, self.session)
