...
from .exceptions import AuthenticationError, NetworkError, ParsingError


# headers = {
#   'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) \
#       Chrome/110.0.0.0 Safari/537.36',
#   'Referer': 'https://connect-web.froeling.com/'
#   }
...
class Session:
    ...
    def set_token(self, token: str):
        ...
        try:
            ...
            raise ValueError("Token is in an invalid format.") from exc
    ...
    async def login(self) -> dict:
        ...
        async with await self.clientsession.post(LOGIN, json=data) as res:
            if res.status != 200:
                raise AuthenticationError(f'Server returned {res.status}: "{await res.text()}"')
                ...
                if res.status == 401:
                    if self.auto_reauth:
                        if self._reauth_previous:
                            raise AuthenticationError("Reauth did not work.", await res.text())
                        ...
                        raise AuthenticationError("Request not authorized: ", await res.text())
                else:
                    error_data = await res.text()
                    raise NetworkError("Unexpected return code", status=res.status, url=res.url, res=error_data)

        except json.decoder.JSONDecodeError as exc:
            raise ParsingError(exc.msg, exc.doc, exc.pos, url) from exc
